...
class CoopPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `coop`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM coop WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO coop (date, title, link, content)"
